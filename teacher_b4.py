...
from toolbox.models.encoder.mix_transformer import mit_b5, mit_b2, mit_b3, mit_b4
...
class edge_fea(nn.Module):
    def __init__(self, f_c):
        super().__init__()
        self.embeding = nn.Sequential(nn.Conv2d(1, 1, 7, 4, 3),
                                      nn.BatchNorm2d(1),
                                      nn.ReLU())
        self.dee1 = dee(f_c[0])
        self.dee2 = dee(f_c[1])
    ...
    def forward(self, dep, fea_list):
        d = self.embeding(dep)
        d1, f1 = self.dee1(d, fea_list[0])
        d2, f2 = self.dee2(d1, self.up2(fea_list[1]))
        d3, f3 = self.dee3(d2, self.up4(fea_list[2]))
        d4, f4 = self.dee4(d3, self.up8(fea_list[3]))
...
class head(nn.Module):
    ...
    def forward(self, fea_list):
        f0 = self.c0(fea_list[0])
        f1 = self.c1(fea_list[1])
        f2 = self.c2(fea_list[2])
        f3 = self.c3(fea_list[3])
...
class seg_seg(nn.Module):
    ...
    def forward(self, rgb, dep=None):
        ...
        e_l, fea_list_r = self.edge_fea(dep, fea_list)
        edge = self.edge(e_l)
